...
class RobotPortrait:
    def __init__(self, root):
        ...
        # Update canvas scrollable region when frame is resized
        self.scrollable_frame.bind(
            "<Configure>", lambda e: self.canvas.config(scrollregion=self.canvas.bbox("all"))
        )

        # This will make sure canvas window follows canvas size
        self.canvas.bind(
            "<Configure>",
            lambda e: self.canvas.itemconfig(self.canvas_window, width=e.width)
        )

        # Pack the canvas in root
        self.canvas.pack(side=LEFT, fill=BOTH, expand=True)

        # Now you can call create_widgets()
        self.create_widgets()
    ...
    def main_page_widg(self):
        self.title_label = tk.Label(self.scrollable_frame, text="Welcome to your Robot Portrait Generator", font=("Baskerville", 24), bg="#eefbfb", fg="#722f37")
        self.title_label.pack(pady=20)

        self.start_button = tk.Button(self.scrollable_frame, text="Start", command=self.portrait_generator, font=("Baskerville", 15))
    ...
    def portrait_generator(self):
        ...
        self.label_instruction = tk.Label(self.scrollable_frame, text="Choose the 4 most look-alike portraits by clinking on it.", font=("Baskerville", 12), bg="#eefbfb", fg="#722f37")
        self.label_instruction.pack(pady=10)

        # Elements choosen by the user
        self.frame_choices = tk.Frame(self.scrollable_frame, bg="#eefbfb")
        ...
        # Generator button
        self.btn_gen = tk.Button(self.scrollable_frame, text="Generate !", command=self.generate_portraits, bg="#26619c", fg="#faebd7", font=("Baskerville", 10, "bold"))
        self.btn_gen.pack(pady=10)

        # Portraits zone
        self.frame_portraits = tk.Frame(self.scrollable_frame, bg="#eefbfb")
        self.frame_portraits.pack()

        # Best choice zone
        self.label_selected = tk.Label(self.scrollable_frame, text="More accurate portrait :", font=("Arial", 12), bg="#eefbfb", fg="#722f37")
        self.label_selected.pack()

        self.canvas_selected = tk.Canvas(self.scrollable_frame, width=128, height=128, bg="#d9f6f6")
        self.canvas_selected.pack(pady=10)

        # Buttons to move on
        self.frame_btn = tk.Frame(self.scrollable_frame, bg="#f0f0f0")
        ...
        self.frame_right = tk.Frame(self.scrollable_frame, bg="#eefbfb")  # New frame for right alignment
    ...
    def previous_step(self):
        "Back to the previous selection step"

        if not self.history:
            return

        last_selection = self.history.pop()
        self.selected_portraits = last_selection

        for widget in self.frame_portraits.winfo_children():
            widget.destroy()

        self.display_images(self.selected_portraits, boole=False)
